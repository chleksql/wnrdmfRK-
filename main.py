questions = [
    {"q": "🌙 몇 시에 자나요?", "opts": ["9시 이전", "10~11시", "12시 이후", "새벽까지"]},
    {"q": "🛌 하루 수면 시간?", "opts": ["8시간 이상", "6~8시간", "4~6시간", "4시간 이하"]},
    {"q": "🍔 자주 먹는 음식?", "opts": ["채소/과일", "고기/단백질", "인스턴트", "패스트푸드"]},
    {"q": "👯‍♂️ 친구 자주 만나나요?", "opts": ["매일", "주 2~3회", "한 달 1회", "거의 못 만남"]},
    {"q": "🏃‍♀️ 운동 얼마나?", "opts": ["매일", "주 2~3회", "가끔", "거의 안 함"]},
    {"q": "🚗 운전 자주?", "opts": ["안 함", "가끔", "자주", "매일"]},
    {"q": "🏠 안전장치 상태?", "opts": ["완벽", "대부분 있음", "부족", "거의 없음"]},
    {"q": "😰 스트레스 빈도?", "opts": ["거의 없음", "가끔", "자주", "항상"]},
    {"q": "💊 약물 사용?", "opts": ["안 함", "가끔", "자주", "과다복용"]},
    {"q": "🧠 정신 상태?", "opts": ["좋음", "보통", "불안/우울", "심각"]},
]

def predict(answers):
    risk = {"심혈관":0,"암":0,"교통사고":0,"자살":0,"자연사":1,"사고":0,"약물":0}
    if answers[1]=="4시간 이하" or answers[4]=="거의 안 함": risk["심혈관"]+=3
    if answers[2] in ["인스턴트","패스트푸드"]:
        risk["암"]+=2; risk["심혈관"]+=1
    if answers[3]=="거의 못 만남": risk["자살"]+=2
    if answers[7] in ["자주","항상"]: risk["자살"]+=2
    if answers[9] in ["불안/우울","심각"]: risk["자살"]+=3
    if answers[5] in ["자주","매일"]: risk["교통사고"]+=3
    if answers[6] in ["부족","거의 없음"]: risk["사고"]+=2
    if answers[8]=="과다복용": risk["약물"]+=4; risk["자살"]+=2
    key=max(risk,key=risk.get)
    msgs = {
        "심혈관":"❤️‍🩹 심장 건강 조심! 운동과 수면을 챙기세요!",
        "암":"🎗️ 식습관 주의! 건강한 음식으로 바꿔봐요!",
        "교통사고":"🚦 운전 조심! 안전벨트와 헬멧 꼭 착용!",
        "자살":"🧠 마음 건강 중요! 힘들 땐 주변에 도움 요청하세요!",
        "자연사":"🌿 평화로운 자연사 가능성이 높아요!",
        "사고":"⚠️ 안전에 신경 쓰세요! 집에서도 조심!",
        "약물":"💉 약물 과다복용 위험! 전문가 상담 추천해요!",
    }
    return msgs[key]

def run():
    print("💀 미래 죽음 예측 설문에 오신 걸 환영합니다! 💀")
    name = input("이름을 알려주세요: ").strip()
    print(f"\n{name}님, 시작할게요! 아래 보기 중에서 답을 그대로 입력하세요.\n")

    ans = []
    for i, q in enumerate(questions,1):
        print(f"{i}. {q['q']}")
        print("보기:", ", ".join(q["opts"]))
        while True:
            reply = input("답 입력: ").strip()
            if reply in q["opts"]:
                ans.append(reply)
                print()
                break
            else:
                print("😅 보기 중에서 정확히 입력해주세요!")

    print("🩺 분석 중... 잠시만요!\n")
    print(f"🩸 {name}님의 미래 사망 원인 예측:")
    print(predict(ans))
    print("\n🙏 삼가 고인의 명복을 빕니다 🙏\n")
    print("💀 참여해 주셔서 감사합니다! 💀")

if __name__=="__main__":
    run()
